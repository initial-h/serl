...
# 引入我们新写的 DrQv2Agent
from serl_launcher.agents.continuous.drq_v2 import DrQv2Agent
...
flags.DEFINE_string("agent", "drq_v2", "Name of agent.")
flags.DEFINE_string("exp_name", None, "Name of the experiment for wandb logging.")
flags.DEFINE_integer("max_traj_length", 1000, "Maximum length of trajectory.")
flags.DEFINE_integer("seed", 42, "Random seed.")
flags.DEFINE_bool("save_model", False, "Whether to save model.")
flags.DEFINE_integer("batch_size", 256, "Batch size.")
flags.DEFINE_integer("critic_actor_ratio", 1, "critic to actor update ratio (Usually 1 for DDPG/DrQ-v2).")

flags.DEFINE_integer("max_steps", 1000000, "Maximum number of training steps.")
flags.DEFINE_integer("replay_buffer_capacity", 200000, "Replay buffer capacity.")

flags.DEFINE_integer("random_steps", 2000, "Sample random actions for this many steps (DrQ-v2 uses random warmup).")
flags.DEFINE_integer("training_starts", 2000, "Training starts after this step.")
flags.DEFINE_integer("steps_per_update", 1, "Number of steps per update (DDPG usually updates every step).")
...
# AgentLace ports / store name (useful to avoid collisions when running multiple examples).
flags.DEFINE_integer("port", 5488, "Learner request-reply port for AgentLace.")
flags.DEFINE_integer("broadcast_port", 5489, "Learner broadcast port for AgentLace.")
_DEFAULT_STORE_NAME = "async_drq_v2_sim"
flags.DEFINE_string("store_name", _DEFAULT_STORE_NAME, "AgentLace data store name.")
...
# DrQ-v2 Exploration Hyperparameters
flags.DEFINE_float("stddev_start", 1.0, "Initial exploration noise stddev.")
flags.DEFINE_float("stddev_end", 0.1, "Final exploration noise stddev.")
flags.DEFINE_integer("stddev_schedule_steps", 500000, "Steps to decay stddev over.")


devices = jax.local_devices()
num_devices = len(devices)
sharding = jax.sharding.PositionalSharding(devices)
# On single-device (e.g., Mac CPU), avoid passing a sharding annotation into
# `jax.device_put` (it can trigger rank/sharding incompatibility errors).
replicated_device = sharding.replicate() if num_devices > 1 else None
...
def _normalize_store_name(name: str) -> str:
    # ZMQ/AgentLace store names are plain strings; keep it simple and filesystem-safe.
...
def actor(agent: DrQv2Agent, data_store, env, sampling_rng):
    """
    ...
    for step in tqdm.tqdm(range(FLAGS.max_steps), dynamic_ncols=True):
        ...
        # --- DrQ-v2 Exploration Schedule ---
        # Linear decay for stddev
        stddev = FLAGS.stddev_start - (FLAGS.stddev_start - FLAGS.stddev_end) * min(1.0, step / FLAGS.stddev_schedule_steps)
        # -----------------------------------
        ...
        with timer.context("sample_actions"):
            if step < FLAGS.random_steps:
                ...
                actions = agent.sample_actions(
                    ...
                    seed=key,
                    deterministic=False, # Must be False to apply noise
                    stddev=stddev,       # Pass the scheduled stddev
                )
        ...
        if step % FLAGS.eval_period == 0:
            with timer.context("eval"):
                evaluate_info = evaluate(
                    # Evaluation is always deterministic (stddev=0)
        ...
        if step % FLAGS.log_period == 0:
            stats = {"timer": timer.get_average_times(), "exploration_stddev": stddev}
            client.request("send-stats", stats)
...
def learner(
    ...
    # Create server
    # Use the same store name as the actor to avoid cross-experiment collisions.
    ...
    for step in tqdm.tqdm(range(FLAGS.max_steps), dynamic_ncols=True, desc="learner"):
        # DrQ-v2 usually does 1 update per step, so critic_actor_ratio is typically 1
        # But we keep logic flexible.
        ...
        with timer.context("train"):
            ...
            # Using update_high_utd for efficiency even if utd=1
...
def main(_):
    ...
    # Make default store name unique-ish to avoid cross-example collisions when users
    # run multiple actors/learners without overriding flags.
    if FLAGS.store_name == _DEFAULT_STORE_NAME:
        FLAGS.store_name = _normalize_store_name(
            f"{_DEFAULT_STORE_NAME}_{FLAGS.env}_{FLAGS.seed}_{FLAGS.port}"
        )
    print_green(
        f"AgentLace config: store_name={FLAGS.store_name}, port={FLAGS.port}, "
    ...
    # --- Create DrQ-v2 Agent ---
    agent: DrQv2Agent = DrQv2Agent.create_drq_v2(
        rng=rng,
    ...
    if num_devices == 1:
        agent: DrQv2Agent = jax.device_put(jax.tree_map(jnp.array, agent))
    else:
        agent: DrQv2Agent = jax.device_put(
