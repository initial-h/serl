...
class MemoryEfficientReplayBuffer(ReplayBuffer):
    ...
    def insert(self, data_dict: DatasetDict):
        ...
        if not isinstance(data_dict.get("observations", None), dict) or not isinstance(
            data_dict.get("next_observations", None), dict
        ):
            raise TypeError(
                "MemoryEfficientReplayBuffer expects transition['observations'] and "
                "transition['next_observations'] to be dict-like (so we can pop pixel keys). "
                f"Got observations={type(data_dict.get('observations', None))}, "
                f"next_observations={type(data_dict.get('next_observations', None))}. "
                f"Expected pixel_keys={self.pixel_keys}. "
                "This usually means the Actor and Learner are running different envs/wrappers, "
                "or a different example's actor connected to this learner (same AgentLace ports / store_name). "
                "Fix by ensuring actor+learner use the same example/env/wrappers AND use a unique "
                "--port/--broadcast_port/--store_name tuple per experiment."
            )
