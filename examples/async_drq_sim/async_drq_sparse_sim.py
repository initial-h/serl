...
flags.DEFINE_string("reward_type", "dense", "Reward type: 'dense' or 'sparse'.")
...
_DEFAULT_STORE_NAME = "async_drq_sim"
...
def main(_):
    ...
    if FLAGS.render:
        env = gym.make(FLAGS.env, render_mode="human", reward_type=FLAGS.reward_type)
    else:
        env = gym.make(FLAGS.env, reward_type=FLAGS.reward_type)
    ...
    if num_devices == 1:
        agent: DrQAgent = jax.device_put(jax.tree_map(jnp.array, agent))
    else:
